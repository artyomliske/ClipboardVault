...
1. Установите полноценное приложение **Xcode** и откройте `ClipboardVault.xcodeproj`.
2. Выберите цель **Clipboard Vault** и устройство запуска **My Mac**.
3. Если Xcode запросит подпись, выберите свою команду разработки в разделе **Signing & Capabilities**.
4. Нажмите `Cmd + R`. После запуска значок приложения появится в строке меню.

На подключённом Mac установлены Apple Command Line Tools, но не выбрана полная среда Xcode. Исходники проходят проверку типизации и компиляцию доступным компилятором Swift; графическую сборку приложения следует выполнить в Xcode.
...
Поддерживаются обычный текст и изображения, которые macOS может представить как `NSImage`. Форматированный текст, документы, произвольные файлы, исключения для выбранных приложений, глобальная горячая клавиша, запуск при входе и синхронизация с iCloud отложены до следующих версий.
